      ...
      connection.query("SELECT * FROM legends_table", function(err, result) {
         if (err) {
            ...
            result.forEach(function(row) {
               row.content = `<div><p>${row.content.replace(
                  /\s\s/g,
                  "</p><p>"
               )}</p></div>`.replace(/<p><\/p>/g, "");
            });
...
router.post("/auth", function(req, res) {
   req.getConnection(function(err, connection) {
      ...
      if (username && password) {
         connection.query(
            `SELECT * FROM accounts WHERE username = ? AND aes_decrypt(password, 'admin_password') = ?`,
            [username, password],
...
router.get("/admin_mysteries", function(req, res, next) {
   if (req.session.loggedin) {
      req.getConnection(function(err, connection) {
         ...
         connection.query("SELECT * FROM mysteries_table", function(
            err,
            result
         ) {
            if (err) {
...
router.post("/add_mystery", function(req, res) {
   ...
   req.getConnection((err, connection) => {
      const query = connection.query(
         "INSERT INTO mysteries_table set ?",
         data,
         (err, newEntry) => {
...
router.get("/delete_legend/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "DELETE FROM legends_table WHERE legend_id = ?",
         [id],
         (err, rows) => {
            res.redirect("/admin_legends");
         }
...
router.get("/delete_mystery/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "DELETE FROM mysteries_table WHERE mystery_id = ?",
         [id],
         (err, rows) => {
            res.redirect("/admin_mysteries");
         }
...
router.get("/delete_video/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "DELETE FROM videos_table WHERE video_id = ?",
         [id],
         (err, rows) => {
            res.redirect("/admin_videos");
         }
...
router.get("/edit_legends/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "SELECT * FROM legends_table WHERE legend_id = ?",
         [id],
         function(err, rows) {
...
router.get("/edit_mysteries/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "SELECT * FROM mysteries_table WHERE mystery_id = ?",
         [id],
         function(err, rows) {
...
router.get("/edit_videos/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "SELECT * FROM videos_table WHERE video_id = ?",
         [id],
         function(err, rows) {
...
router.post("/update_legends/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "UPDATE legends_table set ? where legend_id = ?",
         [newEntry, id],
         (err, rows) => {
...
router.post("/update_mysteries/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "UPDATE mysteries_table set ? where mystery_id = ?",
         [newEntry, id],
         (err, rows) => {
...
router.post("/update_videos/:id", function(req, res) {
   ...
   req.getConnection(function(err, conn) {
      conn.query(
         "UPDATE videos_table set ? where video_id = ?",
         [newEntry, id],
         (err, rows) => {
